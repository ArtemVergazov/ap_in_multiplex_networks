...
lint One_Step_of_Multiplex_HDA (NETWORK &network1, NETWORK &network2, class MTRand *RandNumb)
{
    ...
    for (lint i=ZERO ; i<network1.numNodes ; i++)
    {
        if (network1.gccRoot == network1.componentID[i])
        {
            ...
            //score = (degree1>degree2)?degree1:degree2; 
                                  ...
                                  const bool printMCC)
{
    ...
    printf("\n***********************************\n");
    printf("Process: High Degree Adaptive [cost type=%s] \n", costType.c_str());
    ...
    sprintf(outputFileName1, "%s/HDA_%s._SEED_%ld._%s._%s.gmcc", outputAddress.c_str(), costType.c_str(), seed, 
        network1.name.c_str(), network2.name.c_str());
    ...
    sprintf(outputFileName2, "%s/HDA_%s._SEED_%ld._%s._%s.mcc",
